...
object TiSqlScd {
  ...
  def createSCDTbale(srcDbName: String, srcTblName: String, tarDbName: String, tarTblName: String, v_spark: SparkSession, func: String => Int) {
    ...
    val tarCnt = v_spark.sql("select count(1) as cnt from dw.dw_table_columns where table_schema='" + tarDbName + "' and table_name='" + tarTblName + "'")

    if (tarCnt.first()(0).toString=="0") {
      ...
      var sqlDelete = "drop table if exists "+ tarDbName + "." + tarTblName +";"
      func(sqlDelete)
      var sqlCreate = "create table if not exists " + tarDbName + "." + tarTblName + " ("
      for (r <- dfCollect) {
      ...
      sqlAlter += "alter table " + tarDbName + "." + tarTblName + " add column dt_start_date date;"
      sqlAlter += "alter table " + tarDbName + "." + tarTblName + " add column is_current int;"
      sqlAlter += "alter table " + tarDbName + "." + tarTblName + " add column dt_end_date date;"
      func(sqlAlter)
    }
    ...
    if (srcColCnt + cnt != tarColCnt) {
      ...
      val sqlBak1 = "drop table if exists " + tarDbName + "." + tarTblName + "1;"
      func(sqlBak1)
    }
  }
  //将SOURCE表/SCD表数据MD5放到临时表
  def getDataFrame(tableName: String, primaryKey: String, v_spark: SparkSession): DataFrame = {
    val sqlCol = v_spark.sql("select * from " + tableName + " where 1=0").columns
    var sqlParse = ""
    if (!sqlCol.contains("etl_time")) {
      sqlParse = "select *,now() as elt_time," + primaryKey + " as primaryKey from " + tableName
    }
    else {
      sqlParse = "select *," + primaryKey + " as primaryKey from " + tableName
    }
  ...
  def getNoChangedDataFrame(df1: DataFrame, df2: DataFrame,df3: DataFrame, v_spark: SparkSession): DataFrame = {
    ...
    val df5 = v_spark.sql(
      """
    select a.*,a.dt_start_date as dt_start_date_1,a.is_current as is_current_1,a.dt_end_date as dt_end_date_1
    from df_2_tmp a
    ...
    """)
    val df6 = df5.drop("primaryKey", "is_current", "dt_start_date", "dt_end_date").withColumnRenamed("dt_start_date_1", "dt_start_date").withColumnRenamed("is_current_1", "is_current")
      .withColumnRenamed("dt_end_date_1", "dt_end_date")
    df6
  }
  ...
  def getInvalidDataFrame(df1: DataFrame, v_spark: SparkSession): DataFrame = {
    ...
    //df1.show()
    df1.createOrReplaceTempView("df_1_tmp")
    val df2 = v_spark.sql("""select a.*,dt_start_date as dt_start_date_1,is_current as is_current_1,dt_end_date as dt_end_date_1  from df_1_tmp a where a.is_current = '0'""")
    val df3 = df2.drop("primaryKey", "is_current", "dt_start_date", "dt_end_date").withColumnRenamed("dt_start_date_1", "dt_start_date").withColumnRenamed("is_current_1", "is_current")
      .withColumnRenamed("dt_end_date_1", "dt_end_date")
    df3
  }
  ...
  def updateInvalidDataFrame(df1: DataFrame, df2: DataFrame, v_spark: SparkSession): DataFrame = {
    ...
    val df4 = df3.drop("is_current", "dt_end_date", "primaryKey")
    df4.createOrReplaceTempView("df_4_tmp")
    val df5 = v_spark.sql("""select a.*,'0' as is_current,date_format(now(),'YYYY-MM-dd') as dt_end_date from df_4_tmp a""")
  ...
  def updateValidDataFrame(df1: DataFrame, df2: DataFrame, df3: DataFrame, v_spark: SparkSession): DataFrame = {
    ...
    df3.createOrReplaceTempView("df_3_tmp")
    val df4 = v_spark.sql(
      """select a.*,date_format(now(),'YYYY-MM-dd') as dt_start_date,'1' as is_current,'9999-12-31' as dt_end_date from df_1_tmp a
  ...
  def loseInValidDataFrame(df1: DataFrame, df2: DataFrame, v_spark: SparkSession): DataFrame = {
    ...
    df1.createOrReplaceTempView("df_1_tmp")
    df2.createOrReplaceTempView("df_2_tmp")
    val df3 = v_spark.sql("""select a.*,a.dt_start_date as dt_start_date_1 from df_2_tmp a where not exists (select * from df_1_tmp b where a.primaryKey=b.primaryKey) and a.is_current = '1'""")
    val df4 = df3.drop("is_current", "dt_end_date", "primaryKey", "dt_start_date").withColumnRenamed("dt_start_date_1", "dt_start_date")
    df4.createOrReplaceTempView("df_4_tmp")
    val df5 = v_spark.sql("""select a.*,'0' as is_current,date_format(now(),'YYYY-MM-dd') as dt_end_date from df_4_tmp a""")
    df5
  }
